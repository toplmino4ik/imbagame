...
const SUPERNOVA = {
    ...
    maxFlare() {
        ...
        if (player.sn.tier.gte(4)) x = x.mul(Decimal.pow(100000,player.sn.tier.sub(3)))
    ...
    flareGain() {
        ...
        } else tmp.sf_overflow = E(10)

        return x
    },
    eclipse: {
        req(offest = 0) { return Decimal.pow(1.00000005,player.sn.eclipse.sub(offest).scale(tmp.distant_eclipse,3,3).scale(tmp.scale_eclipse,2,0).add(1)).sub(1).mul(2000) },
