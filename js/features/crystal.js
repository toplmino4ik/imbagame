MAIN.crystal = {
    gain() {
        ...
        let x = Decimal.pow(2.1+getASEff('crystal'),l).mul(l).mul(player.bestPP.div(1e7).max(1).root(3))
...
RESET.crystal = {
    ...
    req: ()=>player.level>=79,
    reqDesc: ()=>`Reach Level 79 to Crystallize.`,
    ...
    ctn: [
        {
            ...
            cost: i => Decimal.pow(.002,scale(E(i),1e6,2,0)).mul(4).ceil(),
            ...
            title: "XP III",
            desc: `Increase XP gain by <b class="green">+300%</b> per level. This effect is increased by <b class="green">50%</b> for every <b class="yellow">25</b> levels.`,
            ...
            effect(i) {
                let x = Decimal.pow(6,i.div(25).floor()).mul(i.div(2).add(1))
...
MAIN.oil = {
    gain() {
        ...
        let l = player.tier
        let x = Decimal.pow(3.9,l).mul(l).mul(player.bestAP.div(1e12).max(1).root(3))
...
RESET.oil = {
    ...
    req: ()=>player.level>=79,
    reqDesc: ()=>`Reach Level 79 to Liquefy.`,
...
MAIN.cloud = {
    gain() {
        ...
        let x = Decimal.pow(3.9,l).mul(l).mul(player.bestNP.div(1e24).max(1).root(3))
...
RESET.cloud = {
    ...
    req: ()=>player.level>=121,
    reqDesc: ()=>`Reach Level 121 to Vaporize.`,
