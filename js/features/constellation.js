RESET.constellation = {
    ...
    req: ()=>player.planetoid.level>=321,
    reqDesc: ()=>`Reach Level 321.`,
    ...
    ctn: [
        {
            ...
            desc: `Increase planetarium gain by <b class="green">+300%</b> per level.<br>This effect is increased by <b class="green">50%</b> every <b class="yellow">25</b> levels.`,

            res: "line",
            icon: ['Curr/Planetoid'],
                
            cost: i => Decimal.pow(1.025,i).mul(5).ceil(),
            bulk: i => i.div(5).max(1).log(1.25).floor().add(1),

            effect(i) {
                let x = Decimal.pow(3,Math.floor(i/25)).mul(i/2+1)
